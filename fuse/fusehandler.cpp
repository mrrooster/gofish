    ...
    connect(obj,&GoogleDriveObject::children,this,[=](QVector<GoogleDriveObject*> children,quint64 requestToken){
        ...
        if (this->inflightOpMap.contains(requestToken)) {
            ...
            if (op.op==ReadDir) {
                ...
                off += fuse_add_direntry(op.req,dirBuff+off,size-off,".",&stbuf,size);
                off += fuse_add_direntry(op.req,dirBuff+off,size-off,"..",&stbuf,size);
                for(GoogleDriveObject *child : children) {
                    QByteArray nameData = child->getName().toUtf8();
                    stbuf.st_ino = child->getInode();
                    off += fuse_add_direntry(op.req,dirBuff+off,size-off,nameData.data(),&stbuf,size);
                }
                if (op.off>=size) {
                    fuse_reply_buf(op.req,dirBuff,0);
                } else {
                    fuse_reply_buf(op.req,dirBuff+op.off,size>op.size?op.size:size);
                }
                delete[] dirBuff;
...
{
    SD("In fuse readdir"<<ino<<size<<off);
